...
-(NSString*) model {
    if (!_model) {
        _model = SAFE_ARC_RETAIN([TTDeviceInfo getPhoneModel]);
    }
    return _model;
}
...
-(NSString*) advertiserId {
    if (!_advertiserId) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *advertiserId = [TTDeviceInfo getAdvertiserId:5];
            if (advertiserId != nil &&
...
-(NSString*) vendorId {
    if (!_vendorId) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *identifierForVendor = [TTDeviceInfo getVendorId:5];
...
+ (NSString*)getAdvertiserId:(int) maxAttempts
{
    ...
    if (ASIdentifierManager && sharedManager && advertisingIdentifier) {
        ...
        if (identifier == nil && maxAttempts > 0) {
            ...
            return [TTDeviceInfo getAdvertiserId:maxAttempts - 1];
        } else {
            return identifier;
        }
    } else {
...
+ (NSString*)getVendorId:(int) maxAttempts
{
    ...
    if (identifier == nil && maxAttempts > 0) {
        // Try again every 5 seconds
        [NSThread sleepForTimeInterval:5.0];
        return [TTDeviceInfo getVendorId:maxAttempts - 1];
    } else {
        return identifier;
    }
}
...
- (NSString*)generateUUID
{
    ...
    NSString *result = [[TTUtils generateUUID] stringByAppendingString:@"R"];
    return result;
}
