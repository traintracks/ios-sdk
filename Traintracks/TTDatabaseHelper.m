...
@interface TTDatabaseHelper()
@end

@implementation TTDatabaseHelper
{
...
static NSString *const LONG_STORE_TABLE_NAME = @"longStore";
...
{
    static TTDatabaseHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[TTDatabaseHelper alloc] init];
    });
    return instance;
}
...
- (id) init
{
    if (self = [super init]) {
        ...
        _databasePath = SAFE_ARC_RETAIN([databaseDirectory stringByAppendingPathComponent:@"traintracks.database"]);
...
- (NSMutableArray*)getEventsFromTable:(NSString*) table upToId:(long long) upToId limit:(long long) limit
{
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([rs next]) {
            ...
            [event setValue:[NSNumber numberWithInt:eventId] forKey:@"eventId"];
